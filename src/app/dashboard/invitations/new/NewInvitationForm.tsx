  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="card p-6">
        ...
        {/* Info paket aktif */}
        {packages
          .filter((p) => p.id === activePkg)
          .map((p) => (
            <p key={p.id} className="mt-3 text-sm text-gray-500">
              Paket <b className="text-gray-800">{p.name}</b> · mulai{" "}
              <span className="font-semibold text-brand-600">{formatIDR(pkgPrice(p))}</span>
              {p.tagline ? ` · ${p.tagline}` : ""}
            </p>
          ))}
